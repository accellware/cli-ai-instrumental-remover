...
| `tuning`             | object              | no       | Optional ORT / CUDA tuning knobs. See below. Omit to keep ORT's defaults.                                                                  |
...
All keys are optional — omit a field to leave the corresponding ORT default in place. Unknown keys are rejected.
...
| --------------------------------------- | --------- | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `intra_threads`                         | integer   | `SessionBuilder::with_intra_threads`             | Threads used inside a single op. No-op if ORT was built with OpenMP — set `OMP_NUM_THREADS` instead.                 |
| `inter_threads`                         | integer   | `SessionBuilder::with_inter_threads`             | Only relevant when `parallel_execution` is `true`.                                                                   |
| `parallel_execution`                    | boolean   | `SessionBuilder::with_parallel_execution`        | Run independent ops concurrently. Higher peak memory.                                                                |
| `memory_pattern`                        | boolean   | `SessionBuilder::with_memory_pattern`            | Predictive memory-pattern optimisation. Disable for dynamic input shapes.                                            |
| `cuda.device_id`                        | integer   | `ort::ep::CUDA::with_device_id`                  | NVIDIA device index.                                                                                                 |
| `cuda.gpu_mem_limit_mb`                 | integer   | `ort::ep::CUDA::with_memory_limit` (× 1024²)     | Caps the CUDA arena, in MiB. Actual usage may exceed this — non-arena allocations are not counted.                   |
| `cuda.arena_extend_strategy`            | enum      | `ort::ep::CUDA::with_arena_extend_strategy`      | `"next_power_of_two"` (default) or `"same_as_requested"` (lower peak, more frequent extends).                        |
| `cuda.cudnn_conv_algo_search`           | enum      | `ort::ep::CUDA::with_conv_algorithm_search`      | `"exhaustive"` (default — slow first run, fastest steady state), `"heuristic"`, or `"default"`.                      |
| `cuda.cudnn_conv_use_max_workspace`     | boolean   | `ort::ep::CUDA::with_conv_max_workspace`         | When `false`, exhaustive search is capped at 32 MiB workspace and may pick a slower kernel.                          |
| `cuda.tf32`                             | boolean   | `ort::ep::CUDA::with_tf32`                       | TensorFloat-32 on Ampere+. Off by default — enable for `MatMul`/`Conv` speed at reduced precision.                   |
| `cuda.prefer_nhwc`                      | boolean   | `ort::ep::CUDA::with_prefer_nhwc`                | Tensor cores often run NHWC layouts faster. Off by default.                                                          |

The `cuda` sub-block is parsed unconditionally but ignored when `execution_provider` is `"cpu"`.
...
`scripts/bench.ps1` runs the GPU docker image (`music-separator:gpu`, built
from `Dockerfile.gpu`) against `tests/raw_vid.mp4` and reports wall-time
statistics:

```powershell
# 3 timed runs after 1 warmup, against the image's baked-in config.cuda.json.
pwsh ./scripts/bench.ps1

# Build the image first.
pwsh ./scripts/bench.ps1 -Build

# A/B test a tuning override: bind-mount a host config.json over /app/config.json.
pwsh ./scripts/bench.ps1 -Config ./bench-configs/tf32-on.json -Iterations 5
```

Per-run logs (full stderr + stdout, including the `[N/5] ... ✓ (Xs)` step
timings printed by the pipeline) land in `out/bench/`.

The latest baseline measurements (hardware, config, mean/min/max/stddev,
real-time factor) are recorded in [`benchmark.md`](../benchmark.md).
